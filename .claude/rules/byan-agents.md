...
| **backend-supabase-engineer** | Nadia | API Bun/Hono, Supabase (PostgreSQL, RLS, Auth, Storage), WebSocket, Drizzle ORM, Shopify |
| **ai-vision-engineer** | Iris | IA vision LLM, prompt engineering, OpenRouter, analyse capillaire, benchmark modeles |
...
- "Implemente le stream microscope" → Hermes recommande `mirror-device-engineer` (Orion)
- "Cree le schema Supabase" → Hermes recommande `backend-supabase-engineer` (Nadia)
- "Design le prompt IA pour l'analyse" → Hermes recommande `ai-vision-engineer` (Iris)
