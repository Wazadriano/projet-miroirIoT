...
- **TDD ancre la qualité** : sur les chemins critiques (synchronisation hors-ligne, parsing des réponses IA), le test est écrit avant le code. C'est mesurable : 34 tests unitaires Vitest + 136 cas E2E Playwright (détail section 4).
...
| **E3 — Workflow séance** | Recherche/création client ; consentement explicite ; capture microscope USB UVC (4 zones max) ; affichage diagnostic | 14 | Fait |
| **E4 — Analyse IA** | Envoi snapshot JPEG au proxy `:3001` ; appel OpenRouter (cloud) ; parsing JSON ; niveau de confiance ; coût ~0,002 EUR/analyse | 10 | Fait |
...
| **E10 — Sécurité & conformité** | Fondations Electron (contextIsolation, preload, IPC) ; remédiation gaps (chiffrement JPEG, CSP, sandbox) ; dossier RGPD Chapitre V | 11 | Partiel |
...
|--------|-------|-----------|----------------------|
| **Unit** | Vitest | 34 | Logique isolée : `api-client.service`, `config.service`, `sync.service` |
...
**Total : 170 cas réels.** Couverture honnête : **3 services sur 8** sont couverts en unitaire. Le service `crm-sync.service.ts` (372 lignes) est à **0 test** — c'est le risque technique n°1 du backlog.

**Dette outillage assumée :** pas encore de CI (`.github/workflows`), pas de configuration de couverture, pas de `playwright.config.ts` versionné, et la config ESLint 9 flat est cassée (lint non opérationnel). 2 CVE à re-vérifier via `npm audit` la veille de l'oral.
