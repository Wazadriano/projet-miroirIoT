...
| 4 | **Microscope / proxy video** | microscope.service.ts (115 LOC), WiFi/TCP par defaut conforme au code (`192.168.34.1:8080`, JHCMD ; ffmpeg H.264->MJPEG sur `localhost:9100`) ; references USB/UVC = vestiges morts ; double-WiFi NON automatise (wifi.service.ts ne gere que wlan0) | **12** | Handshake JHCMD, transcodage ffmpeg, snapshot JPEG, latence ; codec a documenter (cf. encadre Pi5) |
...
| 6 | **Tests / CI / securite** | 178 cas reels (42 unitaires Vitest dont `crypto-vault.service.test.ts` = 7 tests + 136 E2E Playwright sur 4 specs) ; 4/9 services couverts ; crm-sync.service.ts (372 LOC) a **0 test** ; deja en place : CI GitHub Actions (`ci.yml`, audit deps non encore bloquant), playwright.config, ESLint flat config ; **chiffrement au repos AES-256-GCM (cryptoVault) des photos (`.jpg.enc`), de la file de sync et des tokens CRM**. Securite deja active a ne pas re-presenter en gap : sandbox actif (`index.ts:51`), CSP en production (`index.ts:121-143`, garde `if !is.dev`). Gaps restants : securiser le backend mock (PDF, secrets en dur, device_token non hache), audit deps CI bloquant, pgcrypto | **26** | Tiroir le plus lourd : couvrir crm-sync + securiser le backend mock + rendre l'audit deps CI bloquant. npm audit a re-verifier la veille de l'oral |
...
- **Securite** : le chiffrage du tiroir 6 suppose la fermeture des gaps reels restants (securiser le backend mock, audit deps CI bloquant, pgcrypto, durcir `config:getAll`). Securite **deja en place** a valoriser : chiffrement au repos AES-256-GCM via cryptoVault (photos `.jpg.enc`, file de sync, tokens), sandbox actif (`index.ts:51`), CSP en production (`index.ts:121-143`), contextIsolation true, nodeIntegration false, preload contextBridge, IPC typee, CI GitHub Actions (`ci.yml`). `npm audit` a re-verifier la veille de l'oral.
